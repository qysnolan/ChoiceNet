  ...
  .directive('userCard', function($resource, $http) {
    ...
    return {
      ...
      link: function(scope, element, attrs) {
        ...
        // Fetch and return focus areas for user
        var getFocusAreas = function (user) {
          return $http.get('/accounts/'+user.id+'/focus/'+622+'/').then(function (response) {
        ...
        scope.showTeacherInfo = function (user) {
          ...
          if (!user.loadedData) {
            ...
            user.focusAreas = getFocusAreas(user);
          }
